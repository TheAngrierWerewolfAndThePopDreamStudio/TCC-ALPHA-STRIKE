...
document.addEventListener('DOMContentLoaded', function () 
{
    ...
    document.querySelector('.Seta_Avançar').addEventListener('click', function () 
    {
        plusSlides(1);
    });
...
document.addEventListener('DOMContentLoaded', function () 
{
    ...
    document.querySelector('.Seta_Avançar_C2').addEventListener('click', function () 
    {
        slideIndex++;
        mostrarSlide(slideIndex);
    });

    document.querySelector('.Seta_Voltar_C2').addEventListener('click', function () 
    {
        slideIndex--;
        mostrarSlide(slideIndex);
    });
...
function mostrarCards(classe) 
{
    ...
    document.querySelectorAll('.cards_P_Open, .cards_P_Eventos, .cards_P_Treinamentos').forEach(function(el) 
    {
...
document.querySelector('.btn_P_1_Opçao_1').addEventListener('click', function() 
{
    mostrarCards('cards_P_Open');
});

// Adiciona evento de clique para o botão EVENTO
document.querySelector('.btn_P_1_Opçao_2').addEventListener('click', function() 
{
    mostrarCards('cards_P_Eventos');
});

// Adiciona evento de clique para o botão TREINAMENTO
document.querySelector('.btn_P_1_Opçao_3').addEventListener('click', function() 
{
...
function verificarSenha() {
    ...
    var possuiCaractereEspecial = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(senha);
...
// Validação da senha com linhas

function verificarSenha() {
    var senha = document.getElementById('senha_1').value;

    // Esconde todas as linhas de validação
...
// Verifica a força da senha e exibe a linha de validação correspondente
if (senha === '') return; // Se a senha estiver vazia, não exibe nenhuma linha de validação

if (senha.length >= 30 && /[a-zA-Z]/.test(senha) && /[0-9]/.test(senha) && /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(senha)) 
{
    exibirLinhaValidacao(1); // Incrível
} 
else if (senha.length >= 25 && /[a-zA-Z]/.test(senha) && /[0-9]/.test(senha) && /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(senha))
{
    exibirLinhaValidacao(2); // Muito Forte
} 
else if (senha.length > 20 && /[a-zA-Z]/.test(senha) && /[0-9]/.test(senha))
{
    exibirLinhaValidacao(3); // Forte
} 
else if (senha.length > 15 && /[a-zA-Z]/.test(senha) && /[0-9]/.test(senha))
{
    exibirLinhaValidacao(4); // Média
} 
else if (senha.length <= 15 && (/[a-zA-Z]/.test(senha) || /[0-9]/.test(senha) || /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(senha)))
{
    exibirLinhaValidacao(5); // Fraca
}
else if (senha.length <= 10 && (/[a-zA-Z]/.test(senha) || /[0-9]/.test(senha) || /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(senha)))
{
